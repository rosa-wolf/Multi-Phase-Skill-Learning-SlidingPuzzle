...
class FmCallback(BaseCallback):
    ...
    def _check_reward_criterion(self) -> bool:
        ...
        for i in range(out.shape[0]):
            ...
            if self.env.starting_epis:
                ...
                # get unique changes
                num_change = np.unique(np.where(out[i] > 0.08)[1]).shape[0]
        ...
        if change_reward_scheme:
            ...
            if self.n_calls > self.first_change + 250000:
                self.env.end_epis = True
    ...
    def _relabel_rl(self,
        ...
        if start_idx > end_idx:
            idx = np.concatenate((np.arange(start_idx, self.locals["replay_buffer"].buffer_size), np.arange(end_idx + 1)))
        else:

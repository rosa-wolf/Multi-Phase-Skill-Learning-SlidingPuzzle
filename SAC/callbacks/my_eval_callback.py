...
class EvalCallback(EventCallback):
    ...
    def __init__(
        ...
        self.evaluation_game_solving_success_rate: List[List[float]] = []
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if self.log_path is not None:
                ...
                self.evaluation_game_solving_success_rate.append([average_success])
                ...
                np.savez(
                    ...
                    one_step_solving_rate=self.evaluation_game_solving_success_rate,
                    **kwargs,
                )
    ...
    def _solve_puzzles(self):
        ...
        for depth in solution_depth:
            for _ in range(50):
                ...
                goal_sym_state = generate_goal(init_sym_state, depth=depth, puzzle_size=self.gym_env.scene.puzzlesize)
                ...
                if plan is None:
                    ...
                    obs, _ = self.gym_env.reset(skill=plan.pop(0), sym_state_in=init_sym_state)
                    while True:
                    ...
                    if np.all(self.gym_env.scene.sym_state == goal_sym_state):
                        successes.append(True)
                    else:
                        successes.append(False)

        return np.sum(np.array(successes)) / 50.

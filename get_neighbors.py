...
def get_neighbors_dict(puzzle_size):
    """
    Calculates the neibhors of each field, depending on the puzzle size
    """
    neighborlist = {}
    for i in range(puzzle_size[0]):
        for j in range(puzzle_size[1]):
            # look on which side we have a neighbor and mark it in array
            neighbors = {'above': None, 'left': None, 'right': None, 'below': None}
            field = j + (i * puzzle_size[1])
            if i != 0:
                # a neighbor above
                neighbors['above'] = field - puzzle_size[1]
            if j != 0:
                # no neighbor on left
                neighbors['left'] = field - 1
            if j != puzzle_size[1] - 1:
                # a neighbors on right
                neighbors['right'] = field + 1
            if i != puzzle_size[0] - 1:
                # a neighbor below
                neighbors['below'] = field + puzzle_size[1]
